...
虽然长得像 HTML，但它本质上是 JavaScript 的语法糖。浏览器并不能直接运行 JSX，最终需要通过 Babel 等编译器将其转化为标准的 JavaScript 函数调用。

## 书写标签语言

### 只能返回一个根元素

JSX 节点最终会被转译为单一的对象（React.ReactElement），因此必须包含在一个根节点内。如果你不想增加无意义的 HTML 嵌套，可以使用空标签（Fragment）：
...
JSX 标签本质上是一个对象/函数调用（`React.createElement`），如果在一个元素上写多个相同的属性，后面的属性会直接覆盖前面的属性。

```jsx
// ❌ 错误示范：后面的 className 会覆盖前面的
<button className={styles.btn} className={styles.primary}>
  点击
</button>
...
想要应用多个样式类名，需要通过 **字符串拼接** 或 **样式库** 将多个类名合成一个字符串传给 `className`。
...
本质上它们都是 JavaScript 文件，区别更多是**文件命名约定**。习惯使用jsx文件表示React组件
