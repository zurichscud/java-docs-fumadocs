...
你可以传递给 `<img>` 标签的 props 是预定义的（ReactDOM），但是你可以将任何 props 传递给 **你自己定义的** 组件。
...
`Component`不能使用小写，小写会被认为是HTML标签

<Callout title="注意">

现代React更倾向于传递ReactElement
...
然而，props 是 不可变的。当一个组件需要改变它的 props（例如，响应用户交互或新数据）时，它不得不“请求”它的父组件传递 **不同的 props**。