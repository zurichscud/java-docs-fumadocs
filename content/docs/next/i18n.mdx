...
![](https://markdown-lai.oss-cn-hangzhou.aliyuncs.com/marktext1788752261960.png)

在 Next.js (App Router)中，业界最标准的做法是将两者组合使用：

1. 用 `negotiator` 解析 HTTP `Accept-Language` 请求头，拿到**按权重排序的客户端语言列表**。

2. 用 `@formatjs/intl-localematcher` 将该列表与**服务端支持的语言列表**进行符合 Web 标准的算法匹配。
